...
//
// persist=false strips maxAge/expires so the browser treats these as
// session cookies (cleared when the browser closes) instead of persisting
// for the full refresh-token lifetime — this is what actually implements
// "Remember me": the token itself is still valid server-side either way,
// this only controls whether the browser keeps holding onto it.
...
export async function POST(request) {
  ...
  try {
    ...
    const { password, recaptchaToken, rememberMe } = body;
    email = typeof body?.email === "string" ? body.email.trim().toLowerCase() : "";
    // FIX: the login page sends this checkbox value but it was never read -
    // every login got identical cookie persistence regardless of what the
    // person checked.
    ...
    if (totpFactor) {
      ...
      return applyCookies(response, pendingCookies, persist);
    }
