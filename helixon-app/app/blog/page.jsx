...
const CATEGORIES = ["All", "Recruiting tips", "Product updates", "Industry"];
    ...
    <main className="min-h-screen" style={{ background: "var(--mist)" }}>
      ...
      <section className="max-w-[1100px] mx-auto px-6 pt-16 pb-10 text-center">
        <span className="inline-flex items-center gap-1.5 text-[10px] font-semibold px-3 py-1.5 rounded-full mb-6" style={{ background: "var(--mint)", color: "var(--forest)" }}>
          <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" /><polyline points="14 2 14 8 20 8" /></svg>
        ...
        <p className="text-sm leading-relaxed max-w-md mx-auto" style={{ color: "#5a7a6a" }}>
          Screening tips, product updates, and what we're learning from agencies using Helixon every day.
        </p>
      </section>
        ...
        <div className="flex items-center justify-center gap-2 flex-wrap">
          {CATEGORIES.map((c) => (
            <button
              ...
              style={{
                background: active === c ? "var(--forest)" : "white",
                color: active === c ? "white" : "#5a7a6a",
                border: active === c ? "1px solid var(--forest)" : "1px solid var(--border)",
              }}
            >
              {c}
            </button>
      ...
      <section className="max-w-[1100px] mx-auto px-6 pb-24">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filtered.map((post) => (
            ...
            >
              <span className="inline-flex text-[9px] font-semibold uppercase tracking-wide px-2.5 py-1 rounded-full mb-4 self-start" style={{ background: "var(--mint)", color: "var(--forest)" }}>
                {post.category}
              </span>
              <h2 className="text-sm font-semibold mb-2 leading-snug" style={{ color: "#13201b", fontFamily: "var(--font-display)" }}>{post.title}</h2>
              <p className="text-xs leading-relaxed mb-4 flex-1" style={{ color: "#5a7a6a" }}>{post.excerpt}</p>
              <div className="flex items-center gap-2 text-[10px]" style={{ color: "#8aaa9a" }}>
                <span>{post.date}</span>
                <span>·</span>
                <span>{post.read}</span>
              </div>
        ...
        <div className="rounded-[20px] px-8 py-14 text-center" style={{ background: "var(--forest)" }}>
          ...
          <form className="flex flex-col sm:flex-row gap-2.5 max-w-sm mx-auto">
            <input
              ...
              style={{ border: "none" }}
            />
            <button type="submit" className="text-sm font-semibold px-5 py-3 rounded-[10px] transition-transform hover:scale-[1.02]" style={{ background: "white", color: "var(--forest)" }}>
              Subscribe
            </button>
          </form>
        </div>
